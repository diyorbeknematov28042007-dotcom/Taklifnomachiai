...
function getToken() {
  return localStorage.getItem('tkn_token');
}

function setToken(token) {
  localStorage.setItem('tkn_token', token);
}

function removeToken() {
  localStorage.removeItem('tkn_token');
}
...
export async function createInvitation(templateId, category, data, customSlug) {
  return request('/invitations', {
    method: 'POST',
    body: JSON.stringify({ templateId, category, data, customSlug }),
  });
}
...
export async function viewInvitation(slug) {
  return request('/invitations/view/' + slug);
}

export async function setSlug(invitationId, customSlug) {
  return request('/invitations/set-slug', {
    method: 'POST',
    body: JSON.stringify({ invitationId, customSlug }),
  });
}
...
export async function sendResponse(invitationId, rsvp, guestCount, message, senderName) {
  return request('/responses', {
    method: 'POST',
    body: JSON.stringify({ invitationId, rsvp, guestCount, message, senderName }),
  });
}

export async function getResponses(invitationId) {
  return request('/responses/' + invitationId);
}
